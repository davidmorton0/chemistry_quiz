class SymbolQuiz
  attr_reader :quiz_type, :quiz_id
  
  ANSWERS = 4
  
  def initialize(quiz_type, quiz_id)
    @quiz_type = quiz_type
    @quiz_id = quiz_id
  end
  
  def make_symbol_quiz
    question_index = select_questions(quiz_type.level).shuffle!
    
    (0..quiz_type.num_questions - 1).each do |x|
      symbol_question(quiz_id, question_index[x])
    end
  end
